...
"""
This bot's purpose: to take text confessions and (anonymously) send them to https://telegram.me/furry_confessions
"""
...
class PandorasBot(telepot.Bot):

    def confess(self, chat_id, msg):

        confession = msg['text'].replace("/Confess", "/confess", 1).replace("/confess@furryConfessionsBot ", "/confess", 1).replace("/confess ", "", 1)
    ...
    def contactAdmin(self, chat_id, msg):
        ...
        toAdmin = msg['text'].replace("/contactAdmin ", "/contactadmin", 1).replace("/contactadmin@furryConfessionsBot ", "/contactadmin", 1).replace("/contactadmin ", "", 1)
    ...
    def ban(self, chat_id, msg):
        ...
        ban_info = re.match('([A-Fa-f0-9]{64})\s(.*)', msg['text'].replace("/ban@furryConfessionsBot ", "").replace("/ban ", ""), re.DOTALL) # ban_info.groups[0] is ID, [1] is $reason
    ...
    def unban(self, chat_id, msg):
        ...
        id_hash = re.match('([A-Fa-f0-9]{64})', msg['text'].replace("/unban@furryConfessionsBot", "").replace("/unban", "").strip()) # ban_info[0] is ID, [1] is $reason
    ...
    def sauce(self, chat_id, msg):
        ...
        self.sendMessage(chat_id, answer + "https://github.com/Arcaena/Furry-Confessions-Bot/", reply_to_message_id=msg['message_id'])
        ...
        if (content_type == 'text'): #I won't take images, because potential for abuse
            ...
            if (chat_type == 'private'):
                ...
                if (msg['text'][0] != "/"):
                    ...
                    if (message_words[0].replace("/confess@furryconfessionsbot", "/confess") == '/confess'):
                        self.confess(chat_id, msg)

                    elif (message_words[0].replace("/contactadmin@furryconfessionsbot", "/contactadmin") == '/contactadmin'):
                        self.contactAdmin(chat_id, msg)

                    elif (message_words[0].replace("/help@furryconfessionsbot", "/help") == '/help'):
                        self.sendMessage(chat_id, "This bot sends a public, anonymous confession to the [Furry Confessions Channel](https://telegram.me/furry_confessions) here on Telegram. \n\nTo send a confession in, just do /confess [confession].\n\nThis bot only accepts messages in private, and will not respond in groups.\n\nMessage @PandorasFox with any bugs, issues, or suggestions.", reply_to_message_id=msg['message_id'], parse_mode='Markdown')

                    elif (message_words[0].replace("/github@furryconfessionsbot", "/github") == '/github'):
                        self.sauce(chat_id, msg)

            elif (chat_type == 'group' and chat_id == -106839678):

                if (message_words[0].replace("/ban@furryConfessionsBot", "/ban") == '/ban'):
                    self.ban(chat_id, msg)
                elif (message_words[0].replace("/unban@furryConfessionsBot", "/unban") == '/unban'):
